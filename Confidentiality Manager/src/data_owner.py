...
# Encrypt a message
def encrypt_data(sender_address, file_to_encrypt, message_id, process_instance_id):
    conn = sqlite3.connect('../databases/data_owner/data_owner.db')
    x = conn.cursor()
    ...
    #file_contents = {}
    #access_policy = {}
    input_policies = block_int.get_stateContract_policies(process_instance_id)
    getfile = api.cat(input_policies)
    getfile = getfile.replace(b'\\', b'')
    getfile = getfile.decode('utf-8').rstrip('"').lstrip('"')
    getfile = getfile.encode('utf-8')
    getfile = getfile.split(b'####')
    input_policies = json.loads(getfile[1].decode('utf-8'))

    #file_name, policy = next(iter(input_policies.items()))
    #file_contents[message_id] = file_to_encrypt
    #temporal = ""
    #for authority_name, authority_address in authorities_names_and_addresses():
    #    temporal = temporal + str(process_instance_id) + '@' + authority_name + ' and '
    #access_policy[file_name] = ('(' + temporal[:-5] + ') and ' + policy)
    #access_policy[message_id] = input_policies[str(message_id)]
    header = []
    ...
    cipher = cryptocode.encrypt(file_to_encrypt, str(key_encrypt))
    dict_pol = {'CipheredKey': ciphered_key_bytes_string, "FileName": message_id, "EncryptedFile": cipher}
