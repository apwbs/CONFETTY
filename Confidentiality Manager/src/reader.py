...
def start(process_instance_id, message_id, slice_id, sender_address):
    ...
    for authority_address in authorities_addresses:
        #print("auth add: ", authority_address)
        y.execute("SELECT * FROM authorities_generated_decription_keys WHERE process_instance=? AND authority_address=? AND reader_address=?",
    ...
    #print("ciphertext_link:", ciphertext_link)
    getfile = api.cat(ciphertext_link)
    ciphertext_dict = json.loads(getfile)
    #print("dizionario1:", ciphertext_dict['metadata']['process_instance_id'])
    #print("dizionario1 type:", type(ciphertext_dict['metadata']['process_instance_id']))
    #print("process instance id", int(process_instance_id))
