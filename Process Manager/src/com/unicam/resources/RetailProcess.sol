...
string orderID;
	...
	string[2] roleList = [ "Retailer", "Customer" ]; 
	string[1] optionalList = ["Producer" ];
...
constructor() public{
	...
	roles["Customer"] = 0xaeD0aBbD8C55caf1247ED157C5b7c7bB4F358354;
	optionalRoles["Producer"] = 0x0000000000000000000000000000000000000000;		//enable the start process
		StartEvent_102vawy();
...
function Message_0b917rc(string memory good, uint amount) public checkRole(roleList[1]){
...
function Message_1xxdwx2(uint price, bool av) public checkRole(roleList[0]){
...
if(currentMemory.av==false){enable("Message_1h3ew61", 5); 
 }
else if(currentMemory.av==true){enable("ExclusiveGateway_1johog7", 6); 
...
function Message_0e75g56(bool availability, uint cost) public checkRole(optionalList[0]){
	require(elements[7].status==State.ENABLED);
	done(7);
currentMemory.availability=availability;
...
function Message_1tq0g6g(string memory shipInfo) public checkRole(optionalList[0]){
...
function Message_1k6i83o() public payable checkRole(roleList[1]) {
...
function Message_0xbt4mx(string memory customerAddres) public checkRole(roleList[1]){
...
function Message_1vdzv5y(string memory orderID) public checkRole(optionalList[0]){
