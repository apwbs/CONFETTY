...
constructor() public{
        elements[1] = Element("StartEvent_1szc68n", State.ENABLED);
         //roles definition
...
} function StartEvent_1szc68n() private {
	require(elements[1].status==State.ENABLED);
	done(1);
	enable("Message_0c819nm",2);

}

function Message_0c819nm(string memory ciaoo) public checkRole(roleList[0]) {
	require(elements[2].status==State.ENABLED);  
	done(2);
currentMemory.ciaoo=ciaoo;
	enable("EndEvent_16ncq28",3);
EndEvent_16ncq28(); 
}

function EndEvent_16ncq28() private {
