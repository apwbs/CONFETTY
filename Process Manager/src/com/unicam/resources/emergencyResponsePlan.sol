...
contract emergencyResponsePlan {
    ...
    function Message_0yif06o(string memory areaToMitigate) public checkRole(roleList[3]) {
        require(elements[14].status == State.ENABLED);
        done(14);
        currentMemory.areaToMitigate = areaToMitigate;
        enable("ParallelGateway_11k670q", 16);
        ParallelGateway_11k670q();
    }

    function Message_1lp1gqg(string memory areaToRescue) public checkRole(roleList[2]) {
        require(elements[15].status == State.ENABLED);
        done(15);
        currentMemory.areaToRescue = areaToRescue;
        enable("ParallelGateway_11k670q", 16);
        ParallelGateway_11k670q();
    }
