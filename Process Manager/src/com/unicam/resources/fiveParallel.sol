...
function ParallelGateway_187d31l() private { 
	...
	enable("ParallelGateway_0b9ym15", 6); 
ParallelGateway_0b9ym15(); 
	enable("EndEvent_0vb0knf", 7); 
EndEvent_0vb0knf(); 
}

function ParallelGateway_0b9ym15() private { 
	require(elements[6].status==State.ENABLED);
	done(6);
	enable("ParallelGateway_08vsiwm", 8); 
ParallelGateway_08vsiwm(); 
	enable("EndEvent_1vc8ok1", 9); 
EndEvent_1vc8ok1(); 
}

function ParallelGateway_08vsiwm() private { 
	require(elements[8].status==State.ENABLED);
	done(8);
	enable("ParallelGateway_0l54jbg", 10); 
ParallelGateway_0l54jbg(); 
	enable("EndEvent_1x3yoaj", 11); 
EndEvent_1x3yoaj(); 
}

function ParallelGateway_0l54jbg() private { 
	require(elements[10].status==State.ENABLED);
	done(10);
	enable("EndEvent_07q56gf", 12); 
EndEvent_07q56gf(); 
	enable("EndEvent_1uee1cw", 13); 
EndEvent_1uee1cw(); 
}

function EndEvent_07q56gf() private {
	require(elements[12].status==State.ENABLED);
	done(12);  }

function EndEvent_1uee1cw() private {
	require(elements[13].status==State.ENABLED);
	done(13);  }

function EndEvent_1x3yoaj() private {
