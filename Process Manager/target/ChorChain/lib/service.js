...
angular.module('homePage.services', []).factory('service',
		[ "$http", function($http) {
			...
			service.chorsiaSettings = function(input){
				return $http.post("http://127.0.0.1:8888/settings/", input);
			}

			service.subscribe_generateRSA = function(input){
				return $http.post('http://127.0.0.1:8888/certification/generate_rsa_key_pair', input);
			}

			service.certification = function(input){
				console.log(input);
				return $http.post('http://127.0.0.1:8888/certification/attributes_certification_and_authorities', input);
			}

			service.chorsiaCertification1 = function(input){
				return $http.post("http://127.0.0.1:8888/certification/readpublickey/", input);
			}

			service.chorsiaCertification2 = function(input){
				return $http.post("http://127.0.0.1:8888/certification/attributecertification/", input);
			}
